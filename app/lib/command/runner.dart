...
final class CommandResponse {
  const CommandResponse(this.message, {this.isError = false, this.printSyntax});
  final bool isError;
...
final class Command {
  ...
  final CommandResponse Function(Command cur, String argument, List<Flag> flags)
  run;
}
...
String syntax(Command cmd) {
  ...
  if (cmd.subCommands.isNotEmpty) {
    ...
    for (final Command sub in cmd.subCommands) {
      final int space = 15 - sub.use.length;
...
Future<void> run({
  ...
  void Function(String arg) addFlag = (arg) {
    ...
    if (!acceptableFlags.contains(flag.name)) {
      stdout.writeln("The given flag with the name ${flag.name} isn't avertable.");
      return;
    }
    flags.add(flag);
  };
  ...
  CommandResponse response = current.run(current, arg, flags);
