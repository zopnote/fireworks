...
expect(voidptr_t) mem_alloc(const u64_t size) {
    ...
    if (ptr) {
        return (expect(voidptr_t)) { .value = ptr };
    }
...
expect(voidptr_t) arena_alloc(arena_t arena[static 1], const u64_t size) {
    if (!arena->data) return (expect(voidptr_t)) {
        .error = error_new( "arena data pointer is invalid" )
    };

    const u64_t aligned_offset = arena->offset + align_bytes_m - 1 & ~(align_bytes_m - 1);
    const u64_t new_offset = aligned_offset + size;
    if (arena->size < new_offset) return (expect(voidptr_t)) {
